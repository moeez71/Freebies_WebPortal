 ...
 * @author afagot
 */
public final class LoyaltyStatsCache 
{
	/**
	 * Unique instance of the cache manager.
	 */
	private static LoyaltyStatsCache instance = new LoyaltyStatsCache();

	/**
	 * Maximum number of LoyaltyStatsBean objects that may be in the cache.
	 */
	private int maxCacheSize = 80;

	/**
	 * The number of LoyaltyStatsBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual LoyaltyStatsBean.
	 */
	...
	*/
	 ...
	 */
	public static LoyaltyStatsCache getInstance() 
	{
		return instance;
	}

	/**
	 * A LoyaltyStatsBean object is passed to this method for addition to the cache.
	 */
	public void addLoyaltyStats(LoyaltyStatsBean bean) 
	{
		synchronized(updateLock)
		{ 
			Integer id = bean.getLoyaltyStatId();
			if (null == cache.get(id)) 
			{
				...
				else 
				{
					...
					if (maxCacheSize < currentCacheSize) 
					{
						...
						cache.remove(newLink.bean.getLoyaltyStatId());
						lru.next = null;
					} 
					else 
					{
			...
			{ 
				...
				fetchLoyaltyStats(id);
			}
		}
	}

	/**
	 * Return the LoyaltyStatsBean associated with the given key.
	 */
	public LoyaltyStatsBean fetchLoyaltyStats(Integer id) 
	{
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) 
		{
			LoyaltyStatsBean bean = null;
			try 
			{
				bean = LoyaltyStatsManagerImpl.getInstance().loadByPrimaryKey(id);
			} 
	...
	/**
	 * Updates LoyaltyStatsBean object in the cache.
	 */
	public void updateLoyaltyStats(LoyaltyStatsBean bean) 
	{
		synchronized(updateLock)
		{
			// check if the given object is present in the cache
			if (isLoyaltyStatsInCache(bean))  
			{
				Integer id = bean.getLoyaltyStatId();			
	...
	/**
	 * Returns true if LoyaltyStatsBean object is in the cache. FALSE otherwise
	 */
	public boolean isLoyaltyStatsInCache(LoyaltyStatsBean bean) 
	{
		Integer id = bean.getLoyaltyStatId();
