 ...
 * @author afagot
 */
public final class AdvertScheduleCache 
{
	/**
	 * Unique instance of the cache manager.
	 */
	private static AdvertScheduleCache instance = new AdvertScheduleCache();

	/**
	 * Maximum number of AdvertScheduleBean objects that may be in the cache.
	 */
	private int maxCacheSize = 80;

	/**
	 * The number of AdvertScheduleBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual AdvertScheduleBean.
	 */
	...
	*/
	 ...
	 */
	public static AdvertScheduleCache getInstance() 
	{
		return instance;
	}

	/**
	 * A AdvertScheduleBean object is passed to this method for addition to the cache.
	 */
	public void addAdvertSchedule(AdvertScheduleBean bean) 
	{
		synchronized(updateLock)
		{ 
			Integer id = bean.getScheduleId();
			if (null == cache.get(id)) 
			{
				...
				else 
				{
					...
					if (maxCacheSize < currentCacheSize) 
					{
						...
						cache.remove(newLink.bean.getScheduleId());
						lru.next = null;
					} 
					else 
					{
			...
			{ 
				...
				fetchAdvertSchedule(id);
			}
		}
	}

	/**
	 * Return the AdvertScheduleBean associated with the given key.
	 */
	public AdvertScheduleBean fetchAdvertSchedule(Integer id) 
	{
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) 
		{
			AdvertScheduleBean bean = null;
			try 
			{
				bean = AdvertScheduleManagerImpl.getInstance().loadByPrimaryKey(id);
			} 
	...
	/**
	 * Updates AdvertScheduleBean object in the cache.
	 */
	public void updateAdvertSchedule(AdvertScheduleBean bean) 
	{
		synchronized(updateLock)
		{
			// check if the given object is present in the cache
			if (isAdvertScheduleInCache(bean))  
			{
				Integer id = bean.getScheduleId();			
	...
	/**
	 * Returns true if AdvertScheduleBean object is in the cache. FALSE otherwise
	 */
	public boolean isAdvertScheduleInCache(AdvertScheduleBean bean) 
	{
		Integer id = bean.getScheduleId();
