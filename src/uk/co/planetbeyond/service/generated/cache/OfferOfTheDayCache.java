 ...
 * @author afagot
 */
public final class OfferOfTheDayCache 
{
	/**
	 * Unique instance of the cache manager.
	 */
	private static OfferOfTheDayCache instance = new OfferOfTheDayCache();

	/**
	 * Maximum number of OfferOfTheDayBean objects that may be in the cache.
	 */
	private int maxCacheSize = 80;

	/**
	 * The number of OfferOfTheDayBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual OfferOfTheDayBean.
	 */
	...
	*/
	 ...
	 */
	public static OfferOfTheDayCache getInstance() 
	{
		return instance;
	}

	/**
	 * A OfferOfTheDayBean object is passed to this method for addition to the cache.
	 */
	public void addOfferOfTheDay(OfferOfTheDayBean bean) 
	{
		synchronized(updateLock)
		{ 
			Integer id = bean.getOfferId();
			if (null == cache.get(id)) 
			{
				...
				else 
				{
					...
					if (maxCacheSize < currentCacheSize) 
					{
						...
						cache.remove(newLink.bean.getOfferId());
						lru.next = null;
					} 
					else 
					{
			...
			{ 
				...
				fetchOfferOfTheDay(id);
			}
		}
	}

	/**
	 * Return the OfferOfTheDayBean associated with the given key.
	 */
	public OfferOfTheDayBean fetchOfferOfTheDay(Integer id) 
	{
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) 
		{
			OfferOfTheDayBean bean = null;
			try 
			{
				bean = OfferOfTheDayManagerImpl.getInstance().loadByPrimaryKey(id);
			} 
	...
	/**
	 * Updates OfferOfTheDayBean object in the cache.
	 */
	public void updateOfferOfTheDay(OfferOfTheDayBean bean) 
	{
		synchronized(updateLock)
		{
			// check if the given object is present in the cache
			if (isOfferOfTheDayInCache(bean))  
			{
				Integer id = bean.getOfferId();			
	...
	/**
	 * Returns true if OfferOfTheDayBean object is in the cache. FALSE otherwise
	 */
	public boolean isOfferOfTheDayInCache(OfferOfTheDayBean bean) 
	{
		Integer id = bean.getOfferId();
