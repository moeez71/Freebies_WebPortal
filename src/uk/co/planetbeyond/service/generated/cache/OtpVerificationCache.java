 ...
 * @author afagot
 */
public final class OtpVerificationCache 
{
	/**
	 * Unique instance of the cache manager.
	 */
	private static OtpVerificationCache instance = new OtpVerificationCache();

	/**
	 * Maximum number of OtpVerificationBean objects that may be in the cache.
	 */
	private int maxCacheSize = 80;

	/**
	 * The number of OtpVerificationBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual OtpVerificationBean.
	 */
	...
	*/
	 ...
	 */
	public static OtpVerificationCache getInstance() 
	{
		return instance;
	}

	/**
	 * A OtpVerificationBean object is passed to this method for addition to the cache.
	 */
	public void addOtpVerification(OtpVerificationBean bean) 
	{
		synchronized(updateLock)
		{ 
			Integer id = bean.getOtpId();
			if (null == cache.get(id)) 
			{
				...
				else 
				{
					...
					if (maxCacheSize < currentCacheSize) 
					{
						...
						cache.remove(newLink.bean.getOtpId());
						lru.next = null;
					} 
					else 
					{
			...
			{ 
				...
				fetchOtpVerification(id);
			}
		}
	}

	/**
	 * Return the OtpVerificationBean associated with the given key.
	 */
	public OtpVerificationBean fetchOtpVerification(Integer id) 
	{
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) 
		{
			OtpVerificationBean bean = null;
			try 
			{
				bean = OtpVerificationManagerImpl.getInstance().loadByPrimaryKey(id);
			} 
	...
	/**
	 * Updates OtpVerificationBean object in the cache.
	 */
	public void updateOtpVerification(OtpVerificationBean bean) 
	{
		synchronized(updateLock)
		{
			// check if the given object is present in the cache
			if (isOtpVerificationInCache(bean))  
			{
				Integer id = bean.getOtpId();			
	...
	/**
	 * Returns true if OtpVerificationBean object is in the cache. FALSE otherwise
	 */
	public boolean isOtpVerificationInCache(OtpVerificationBean bean) 
	{
		Integer id = bean.getOtpId();
