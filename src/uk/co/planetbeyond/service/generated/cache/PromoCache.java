 ...
 * @author afagot
 */
public final class PromoCache 
{
	/**
	 * Unique instance of the cache manager.
	 */
	private static PromoCache instance = new PromoCache();

	/**
	 * Maximum number of PromoBean objects that may be in the cache.
	 */
	private int maxCacheSize = 80;

	/**
	 * The number of PromoBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual PromoBean.
	 */
	...
	*/
	 ...
	 */
	public static PromoCache getInstance() 
	{
		return instance;
	}

	/**
	 * A PromoBean object is passed to this method for addition to the cache.
	 */
	public void addPromo(PromoBean bean) 
	{
		synchronized(updateLock)
		{ 
			Integer id = bean.getPromoId();
			if (null == cache.get(id)) 
			{
				...
				else 
				{
					...
					if (maxCacheSize < currentCacheSize) 
					{
						...
						cache.remove(newLink.bean.getPromoId());
						lru.next = null;
					} 
					else 
					{
	...
	/**
	 * Return the PromoBean associated with the given key.
	 */
	public PromoBean fetchPromo(Integer id) 
	{
		...
		if (null == foundLink) 
		{
			...
			try 
			{
				bean = PromoManagerImpl.getInstance().loadByPrimaryKey(id);
			} 
	...
	/**
	 * Updates PromoBean object in the cache.
	 */
	public void updatePromo(PromoBean bean) 
	{
		synchronized(updateLock)
		{
			// check if the given object is present in the cache
			if (isPromoInCache(bean))  
			{
				Integer id = bean.getPromoId();			
	...
	/**
	 * Returns true if PromoBean object is in the cache. FALSE otherwise
	 */
	public boolean isPromoInCache(PromoBean bean) 
	{
		Integer id = bean.getPromoId();
