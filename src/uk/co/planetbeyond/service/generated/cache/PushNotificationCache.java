...
public final class PushNotificationCache 
{
	 ...
	 */
	private static PushNotificationCache instance = new PushNotificationCache();

	/**
	 * Maximum number of PushNotificationBean objects that may be in the cache.
	 */
	private int maxCacheSize = 80;

	/**
	 * The number of PushNotificationBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual PushNotificationBean.
	 */
	...
	*/
	...
	/**
	 * A PushNotificationBean object is passed to this method for addition to the cache.
	 */
	public void addPushNotification(PushNotificationBean bean) 
	{
		synchronized(updateLock)
		{ 
			Integer id = bean.getNotificationId();
			if (null == cache.get(id)) 
			{
				...
				else 
				{
					...
					if (maxCacheSize < currentCacheSize) 
					{
						...
						cache.remove(newLink.bean.getNotificationId());
						lru.next = null;
					} 
					else 
					{
	...
	/**
	 * Return the PushNotificationBean associated with the given key.
	 */
	public PushNotificationBean fetchPushNotification(Integer id) 
	{
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) 
		{
			PushNotificationBean bean = null;
			try 
			{
				bean = PushNotificationManagerImpl.getInstance().loadByPrimaryKey(id);
			} 
	...
	/**
	 * Updates PushNotificationBean object in the cache.
	 */
	public void updatePushNotification(PushNotificationBean bean) 
	{
		synchronized(updateLock)
		{
			// check if the given object is present in the cache
			if (isPushNotificationInCache(bean))  
			{
				Integer id = bean.getNotificationId();			
	...
	/**
	 * Returns true if PushNotificationBean object is in the cache. FALSE otherwise
	 */
	public boolean isPushNotificationInCache(PushNotificationBean bean) 
	{
		Integer id = bean.getNotificationId();
